// Usamos el fetch nativo de Node.js para interactuar directamente con la base de datos de Firestore vía REST API
const PROJECT_ID = "levelupstore-87d4d"; // Tu Project ID de Firebase
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // URL base de la API REST de Firestore para tu proyecto
    ...
    if (pagosData.documents) {
      for (const doc of pagosData.documents) {
        const docId = doc.name.split('/').pop().replace('ref_', '');
        if (docId.endsWith(referencia)) {
          pagoDocName = doc.name; // Ruta completa para actualizar después
          pagoEncontradoDoc = {
            estado: doc.fields.estado ? doc.fields.estado.stringValue : 'desconocido'
          };
          break;
        }
      }
    ...
    // 2. Consultar stock disponible en "codigos_roblox" filtrando por producto_id y estado disponible
    // Usamos una consulta estructurada (StructuredQuery) de Firestore REST
    const queryBody = {
      structuredQuery: {
    ...
    // 3. Ejecutar las actualizaciones mediante transacciones REST (Commit)
    const commitBody = {
      writes: [
        // Marcar el pin como vendido
        {
          update: {
            name: pinDocName,
            fields: {
        ...
        // Crear el registro en la colección "pedidos"
        {
          update: {
    ...
    // 4. Retornar el código PIN con éxito al frontend
