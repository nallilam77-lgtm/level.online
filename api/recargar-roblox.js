...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // 🛡️ EXCEPCIÓN: Si el pago es insuficiente, lo devolvemos a "Verificado"
    if (dataVerificacion.insuficiente) {
      ...
      return res.status(200).json({ 
        success: false, 
        message: `Pago insuficiente: Encontramos ${formatearVES(dataVerificacion.montoPagado)} Bs, pero este paquete cuesta ${formatearVES(paqueteSeleccionado.precio)} Bs.` 
      });
    }
      ...
      structuredQuery: {
        ...
        where: {
          compositeFilter: {
            op: "AND",
            filters: [
              { fieldFilter: { field: { fieldPath: "producto_id" }, op: "EQUAL", value: { stringValue: String(numeroRobux) } } },
