export default async function handler(req, res) {
  ...
  try {
    ...
    // ==========================================
    // 1. EL CATÁLOGO MAESTRO (Inmune a hackeos web)
    // ==========================================
    const catalogo = {
      "110 Diamantes":   { code: "110_diamonds", precio: 760.00 },
      "220 Diamantes":   { code: "110_diamonds", precio: 1520.00, doble: true },
      "341 Diamantes":   { code: "341_diamonds", precio: 2300.00 },
      "572 Diamantes":   { code: "572_diamonds", precio: 3850.00 },
      "1.166 Diamantes": { code: "1166_diamonds", precio: 7120.00 },
      "2.278 Diamantes": { code: "2398_diamonds", precio: 14100.00 },
      "6.160 Diamantes": { code: "6160_diamonds", precio: 36000.00 }
    };

    const producto = catalogo[paquete];
    
    // Si alguien intenta inyectar un paquete falso
    if (!producto) {
    ...
    // 🛡️ RECHAZO CON FORMATO EXACTO VENEZOLANO
    ...
    // ==========================================
    // 3. INYECTAR LOS DIAMANTES (FAZERCARDS)
    // ==========================================
    ...
    if (resultadoCompra.ok !== true) {
      // Si la API falla, el escudo protector le devuelve el pago al cliente
      ...
      return res.status(400).json({ status: "error", message: "Fallo en el servidor de recargas: " + (resultadoCompra.error || "Mantenimiento temporal.") });
    }
    ...
    // ==========================================
    // 4. QUEMAR EL PAGO Y GUARDAR EL RECIBO
    // ==========================================
